...
TRANSPILE//AI is an AI-assisted code migration workspace for translating source code between programming languages while explaining the decisions behind the rewrite. It is built for the UBC AI 100 course and uses the course AI model for translation and code review.

The public text shell provides:

- Streaming code translation between Python, JavaScript, TypeScript, Java, Go, Rust, C++, C#, Ruby, PHP, Swift, Kotlin, SQL, and Bash
- Inline explanations of language and implementation differences
- AI-generated translation audits with a score, quality signals, and risk flags
- AI-generated target-language test suites
- A context-aware pair-programmer chat that includes the previous 20 user and assistant messages in each request
- Suggested follow-up questions based on the source and translated code
- Built-in examples, local text-file upload, copy/download actions, light mode, and saved translation history

The app is a Flask/SQLite project derived from the GizmoApp starter. The backend remains prefix-aware and the frontend has no build step, so it can run directly in the hosted CodingWorkspace preview or under a deployment path.
...
The project keeps the TRANSPILE//AI text shell and the reusable graphical shell in the same codebase; both share the same backend, API, database, and deployment scripts.
...
- `POST /api/translate` translates a code sample and saves the completed translation
- `POST /api/translate-stream` streams translation output over Server-Sent Events and saves it when complete
- `GET /api/translation-history` returns saved translations from SQLite
- `POST /api/translation-audit` reviews translated code for quality signals and risks
- `POST /api/generate-tests` generates tests for translated code
- `POST /api/chat-suggestions` generates context-specific follow-up questions
- `POST /api/chat` answers questions about the generated code and accepts up to 20 prior conversation messages
