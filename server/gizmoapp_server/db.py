...
def insert_translation(connection: sqlite3.Connection, payload: dict[str, str]) -> dict[str, Any]:
    cursor = connection.execute(
        """
        INSERT INTO app_events (kind, title, detail, payload_json)
        VALUES ('translation', ?, ?, ?)
        """,
        (f'{payload["source_language"]} -> {payload["target_language"]}', payload["source_code"], json.dumps(payload)),
    )
    connection.commit()
    row = connection.execute(
        "SELECT id, created_at FROM app_events WHERE id = ?",
        (cursor.lastrowid,),
    ).fetchone()
    return {**payload, "id": row["id"], "created_at": row["created_at"]}


def fetch_translation_history(connection: sqlite3.Connection, limit: int = 30) -> list[dict[str, Any]]:
    rows = connection.execute(
        "SELECT id, created_at, payload_json FROM app_events WHERE kind = 'translation' ORDER BY id DESC LIMIT ?",
        (limit,),
    ).fetchall()
    history = []
    for row in rows:
        item = json.loads(row["payload_json"])
        item.update({"id": row["id"], "created_at": row["created_at"]})
        history.append(item)
    return history
